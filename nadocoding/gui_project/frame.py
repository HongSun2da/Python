...
# btn_add_file command
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", \
            filetypes=(("PNG 파일","*.png"),("모든 파일","*.*")), \
            initialdir=r"E:\Development\Python")

    for file in files:
        list_file.insert(END, file)

# btn_del_file command
def del_file():
    for index in reversed(list_file.curselection()):
        list_file.delete(index)

# btn_dest_path command
def browse_dest_path():
    folder_selected = filedialog.askdirectory(title="폴더 경로를 선택하세요.")
    if folder_selected == "":
        return
...
def merge_image():
    ...
    msgbox.showinfo("알람", "작업이 완료 되었습니다.")
...
btn_add_file = Button(file_frame, text="파일추가", command=add_file)
btn_add_file.pack(side="left")

btn_del_file = Button(file_frame, text="파일삭제", command=del_file)
...
# 진행 상황 프레임
frame_progress = LabelFrame(root, text="잔행상황")
